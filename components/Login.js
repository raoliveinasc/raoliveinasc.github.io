...
export default function Login({ entryPointStatus }) {
  const [entryPoint, setEntryPoint] = useState('');
  const nameValid = RetrieveName();

  useEffect(() => {
    if (entryPointStatus == "login") {
      setEntryPoint("Entrar")
    } else {
      setEntryPoint("Cadastre-se")
    }
  })

  return (
    <div className="login-bundle">
        <Link className="login-button" href={`/login?propName=${entryPoint}`}>
            ...
            <div className="login-status">{nameValid != "" ? "Olá, " + nameValid: entryPoint}</div>
        </Link>
    </div>
  )
}
